...
-- --------------------------------------------------------------------------
-- 3. Where the leads stop.
--
-- One row, and every column is a step. Read it left to right and the first
-- number that is zero is the reason the Italy board is empty.
--
-- Two more columns first so nothing here can fail on a missing one.
-- --------------------------------------------------------------------------

alter table partners          add column if not exists sees_leads boolean not null default true;
alter table partner_countries add column if not exists tier       text    not null default 'shared';

select
  -- Is the board the portal reads from still there, and does it still make
  -- an agency wait two days on a new lead?
  (to_regclass('public.partner_board') is not null)                    as board_exists,
  coalesce(pg_get_viewdef(to_regclass('public.partner_board'))
             like '%head_start%', false)                               as board_delays_48h,
  ...
  (select bool_or(sees_leads) from partners
    where name = 'ZZ Test Agency')                                     as portal_on,
  (select string_agg(pc.country || ' as ' || pc.tier, ', ')
     from partner_countries pc
     join partners p on p.id = pc.partner_id
    where p.name = 'ZZ Test Agency')                                   as covers,
  (select count(*) from partner_users pu
     join partners p on p.id = pu.partner_id
    where p.name = 'ZZ Test Agency' and pu.status = 'active')          as active_logins;
